...
<template>
  <main>
    <!-- Page Header -->
    <section class="relative overflow-hidden h-[220px]">
      <img
      ...
      />
      <div class="relative z-10 h-full flex flex-col justify-center pl-20 gap-2">
        <p class="font-body text-[11px] font-bold text-amber tracking-[3px] uppercase">ROOMS</p>
        <h1 class="font-heading text-[52px] font-bold text-primary">Find Your Space</h1>
    ...
    <!-- Rooms Grid -->
    <section class="w-full bg-base py-12 px-20 flex flex-col">
      <div
        v-for="room in rooms"
        :key="room.id"
        class="flex items-center gap-7 py-7 border-b border-subtle"
      >
        <img
          :src="room.listImage"
          :alt="room.name"
          class="w-[240px] h-[156px] object-cover rounded-[10px] shrink-0"
        />
        ...
        <NuxtLink
          ...
          class="font-body text-[13px] font-bold rounded-[10px] py-[13px] px-6 shrink-0 transition-all"
        >
          View Room
        </NuxtLink>
      </div>
