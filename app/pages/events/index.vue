...
<template>
  <main>
    <!-- Page Header -->
    <section class="w-full bg-raised flex flex-col gap-3 py-14 px-20 pb-12">
      <p class="font-body text-[11px] font-bold text-coral tracking-[3px] uppercase">EVENTS</p>
      <h1 class="font-heading text-[52px] font-bold text-primary">Community &amp; Culture</h1>
    ...
    <!-- Upcoming Section -->
    <section class="w-full bg-base py-10 px-20 pb-8 flex flex-col">
      <!-- Section header -->
      ...
      <div
        ...
        :key="evt.id"
        class="flex items-center gap-7 py-6 border-b border-subtle"
      >
        <!-- Date block -->
        ...
        <div class="flex-1 flex flex-col gap-1.5">
          <div class="flex items-center gap-2.5">
            ...
            <span class="font-body text-[11px] font-semibold text-muted bg-card rounded-full py-1 px-2.5">
              {{ evt.type }}
            </span>
          </div>
        ...
        <NuxtLink
          :to="`/events/${evt.id}`"
          class="font-body text-[13px] font-bold text-on-amber bg-amber rounded-[10px] py-[10px] px-5 shrink-0 hover:opacity-90 transition-opacity"
        >
          Details
        </NuxtLink>
      </div>
    ...
    <!-- Past Events -->
    <section class="w-full bg-base py-8 px-20 pb-14 flex flex-col gap-0">
      <!-- Section header -->
